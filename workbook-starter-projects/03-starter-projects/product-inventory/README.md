...
```bash
docker run -p 3002:3002 product-inventory-api
```
...
2. Run the Product Inventory microservice container with the `--network` flag:
   ...
   ```bash
   docker run --network my-network -p 3002:3002 product-inventory-api
   ```
...
3. Run the following command to build the image:
   ...
   ```
   This command will build the Docker image using the provided Dockerfile and tag it as `product-inventory-api`.
